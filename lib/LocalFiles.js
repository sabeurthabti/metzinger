...
LocalFiles.prototype.writeFile = function(localPath, data) {
  ...
  return new Promise(function(resolve, reject) {
    fs.writeFile(localPath, data.replace(/^data:image\/png;base64,/,''), 'base64', function(err) {
      if (err) reject(err);
      else resolve( typeof(data) === "string" ? new Buffer(data, 'binary') : data);
    });
  });
}
...
// LocalFiles.prototype.clean = function() {
